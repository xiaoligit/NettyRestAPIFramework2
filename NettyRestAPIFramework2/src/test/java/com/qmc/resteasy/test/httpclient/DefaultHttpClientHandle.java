package com.qmc.resteasy.test.httpclient;
...
import com.qmc.resteasy.utils.MapUtil;
import com.qmc.resteasy.utils.Md5Util;
import com.qmc.resteasy.utils.SignaturGenUtil;
...
public class DefaultHttpClientHandle extends AbstractHttpClientHandle {
	 ...
	 * @throws IOException
	 */
	public static Object httpGet(String url, Map<String, Object> queryParam)
			throws ClientProtocolException, IOException {
		// HttpGet
		HttpGet httpGet = new HttpGet(getRequestUrl(url, queryParam, null));
		// 处理请求结果
	...
	private static String getRequestUrl(String url, Map<String, Object> params,
		...
		String keyCode = (String) params.get("keyCode");
		// 签名
		String signature = null;
		if ("guest".equals(keyCode)) {
			keyCode = Md5Util.process("HelloWorld!Hereisresteasy_demo"
					+ keyCode);
			// 自定义签名算法对参数串和MD5加密后的字符串进行签名
			signature = StringUtil.specCharFilter(SignaturGenUtil.generator(
					readyTosign, keyCode));
		}
		if (sb.indexOf("?") > 0) {
		...
		logger.info("客户端请求的完整URL为:" + sb.toString());
		return sb.toString();
	}
